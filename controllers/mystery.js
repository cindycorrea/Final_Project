...
const createMystery = async(request, response) => {
  const book = new Mystery({title: request.body.title, author: request.body.author, published: request.body.published, pgCount: request.body.pgCount, synopsis: request.body.synopsis, genre: request.body.genre, audience: request.body.audience});
  try { 
    const result = await book.save();
    response.status(201).json(`${result.title} has been added to the Mystery genre.`)
  } catch (error) {
    console.error("Error:", error);
...
const updateMystery = async(request, response) => {
  ...
  try {
    ...
    response.status(201).json(`${result.title} has been updated.`)
  } catch (error) {
    console.error("Error:", error);
    response.status(500).json('Must be a valid id');
  }
